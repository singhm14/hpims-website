...
const Filters = () => {
  ...
  const defaultLab =
    typeof window !== "undefined"
      ? queryString.parse(window.location.search).lab
      : null;
  ...
  const data = useStaticQuery(graphql`
    query {
      allContentfulTeamMembers(
        filter: {
          labs: { elemMatch: { title: { nin: "Dummy Post, DO NOT DELETE" } } }
        }
      ) {
        nodes {
          department
  ...
  data.allContentfulTeamMembers.nodes.map((teamMember) => {
    ...
    teamMember.department &&
      teamMember.department.map((department) => roles.push(department));
  ...
  return (
    <FiltersWrapper filterText="Filter members">
      <StyledFilters>
        <p className="filters__title paragraph--small font-weight--600">
          {role || lab || project ? "Filtered by" : "Filter by"}
        </p>
        ...
        <Dropdown
          ...
          callbackFunction={(event) =>
            handleRoleFilter(getSlug(event.target.innerText))
          }
          resetFunction={() => handleRoleFilter(null)}
        />

        <Dropdown
          label="Lab"
          defaultOption={defaultLab}
          options={labs}
          callbackFunction={(event) =>
            handleLabFilter(getSlug(event.target.innerText))
          }
          resetFunction={() => handleLabFilter(null)}
        />
        ...
        <Dropdown
          ...
          options={projects}
          callbackFunction={(event) =>
            handleProjectFilter(getSlug(event.target.innerText))
          }
          resetFunction={() => handleProjectFilter(null)}
        />
        ...
        <PrimaryExternal
          ...
          className="filters__button bg-hover--blue500 color--blue500 color-hover--white border--blue500 border-hover--blue500"
          href={urlQueryString === "?" ? "/team" : "/team/" + urlQueryString}
          text="Apply Filter"
        />
      </StyledFilters>
    </FiltersWrapper>
